...
/// 搜索命中结果（借鉴 Joplin / nb 的全文搜索）�?class SearchResult {
  const SearchResult({
...
/// 纯本地扫描（listAll 读取全部工程文件），无需索引文件�?/// 规模增长后可�?SQLite 索引（见学习报告 C1 备注）�?class SearchService {
  ...
  /// 搜索 [query]，忽略大小写；命中文字块内容或标题�?  Future<List<SearchResult>> search(String query) async {
    final q = query.trim().toLowerCase();
    if (q.isEmpty) return const [];

    final results = <SearchResult>[];
    // 1) 笔记本：搜索页面标题与文字块内容�?    final notebooks = await _notebookAccessor.listNotebooks();
    for (final nb in notebooks) {
      ...
      // 搜索增强�?026-08-16）：脱敏摘要匹配——加密笔记本可搜摘要
      // （标�?+ 文本�?200 字符——明文脱敏——未解锁也可搜，安全�?      // 51CTO titlePreview/掘金"核心正文加密+必要摘要脱敏"权威模式）�?      else if (nb.searchSummary.toLowerCase().contains(q)) {
    ...
    // 2) 独立画作：搜索标题�?    final metas = await _docStorage.listDocuments();
  ...
  /// 截取命中位置前后的文字片段�?  static String _snippet(String text, int start, int len) {
...
/// 无注入时的默认实现（空访问器）：不依�?notes 具体类，保持 drawing
/// 完全隔离（S4b）。正式装配由 app 层注入真实实现（NotebookAccessorImpl）�?class _DefaultNotebookAccessor implements INotebookAccessor {
  @override
  ...
  Future<String> storeImage(String sourcePath, String pageId) async =>
      throw UnimplementedError('未注入笔记存储，图片保存不可�?);
}
