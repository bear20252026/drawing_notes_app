...
class NotebookStorage implements NotebookRepository, INotebookAccessor {
  ...
  /// M-09 修复（军工审计 2026-08-15）：允许 '-'（与 StorageService/
  ...
  Future<String> _pathFor(String id) async {
    // C-02 修复（专家审计 2026-08-15）：assert-only 校验，release 失效
  ...
  /// 原子写入笔记本文件（不涉及加密判断；由 [save] 调用）
  Future<String> _writeNotebook(Notebook notebook) async {
    if (!isValidId(notebook.id)) {
      throw ArgumentError.value(notebook.id, 'notebook.id', '笔记本ID 不合法');
    }
  ...
  /// C-01 修复（专家审计 2026-08-15）：验证路径位于受管图片目录
  ...
  Future<String> storeImage(String sourcePath, String pageId) async {
    ...
    // 媒体 VFS 双轨（2026-08-16）：新媒体写 VFS 对象（'vfs:' 标记）
    // 解锁会话由 VaultService 注入（s3-encryption-gateway 双读窗口模式）；
    // 旧媒体 DAN 文件保持兼容读
    ...
    // H-03 部分落地（专家审计 2026-08-15）：源文件大小配额（防超大图
    // 资产入库；完整媒体加密——每笔记 DEK + 渲染解密——评估为数据保密
    // 重构专项，涉及渲染管线跨域改造，参考 Inqrypt/heritage 分层加密模式）
    ...
    // H-03 双端接入（专家审计 2026-08-15）：会话密钥已注入（加密笔记
  ...
  Future<String> encryptAndSave(Notebook notebook, String password) async {
    ...
    // 明文 searchSummary（废除默认明文 searchSummary）。未实现 K_note
  ...
  /// 生成恢复密钥信封（U 盘丢失时用 24 位恢复密钥找回主密钥），
