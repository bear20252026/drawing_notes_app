...
class DocumentCodec {
  ...
  DrawingDocument decode(Uint8List bytes) {
    ...
    if (fileVersion is num && fileVersion.toInt() > _version) {
      throw FormatException('文档版本过新（v$fileVersion，当前支持 v$_version），请升级应用后再打开');
    }
    ...
    return DrawingDocument(
      ...
      folder: document['folder'] is String ? document['folder'] as String : '',
    ...
    if (value is List) {
      ...
      for (var i = 0; i < limit; i++) {
        ...
        try {
          ...
          if (totalStrokes > _maxTotalStrokes ||
              totalPoints > _maxTotalPoints) {
            break;
          }
          layers.add(
            Layer(
