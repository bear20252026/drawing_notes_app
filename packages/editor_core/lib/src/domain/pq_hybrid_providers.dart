...
import 'package:pqcrypto/pqcrypto.dart' as pq;
...
// ──────────── 真实 ML-KEM-768 提供者（pqcrypto） ────────────

/// ML-KEM-768 密钥封装提供者（基于 pqcrypto 包的真实实现）。
///
/// 使用 FIPS 203 对齐的 ML-KEM-768 算法，提供 192 位安全级别。
/// 纯 Dart 实现，跨平台（Android/iOS/Windows/Linux/macOS/Web）。
class PqcryptoMlKemProvider implements PqcKemProvider {
  static final pq.KyberKem _kem = pq.PqcKem.kyber768;

  const PqcryptoMlKemProvider();
  ...
  MlKem768KeyPair generateKeyPair() {
    ...
    return MlKem768KeyPair(
      ...
      secretKey: Uint8List.fromList(sk),
    );
  }

  @override
  MlKem768Encapsulation encapsulate(Uint8List publicKey) {
    final (ct, ss) = _kem.encapsulate(publicKey);
  ...
  Uint8List decapsulate({
    ...
    final ss = _kem.decapsulate(secretKey, ciphertext);
    return Uint8List.fromList(ss);
  }
}

// ──────────── 真实 ML-DSA-65 签名提供者（pqcrypto） ────────────

/// ML-DSA-65（Dilithium-3）数字签名提供者（基于 pqcrypto 包的真实实现）。
///
...
class PqcryptoMlDsaProvider implements PqcSignatureProvider {
  ...
  @override
  MlDsa65KeyPair generateKeyPair() {
    final (pk, sk) = pq.MlDsa.generateKeyPair(_params);
  ...
  Uint8List sign({
    ...
    return Uint8List.fromList(
      pq.MlDsa.sign(secretKey, message, _params),
    );
  }
  ...
  bool verify({
    ...
    return pq.MlDsa.verify(publicKey, message, signature, _params);
  }
}
...
class PqHybridSignerEd25519 {
  ...
  static Future<bool> verify({
    ...
    final pp = pqProvider ?? const PqcryptoMlDsaProvider();
