...
文字 overlay 的确定性样式规则由 `EditorTextPresentationStyle` 承接：它将字体族、字号与视图缩放、待办/便利贴颜色、粗体/斜体/下划线/删除线、对齐和可选宽度映射为不可变的 Flutter presentation 值。它只依赖原始展示输入，不持有 `PageTextItem`、`BuildContext`、编辑控制器或回调。`_EditorPageTextOverlays` 继续负责位置转换、选择/删除动画、待办写回、文字拖动、斜杠命令和宽度缩放；旧文档整块样式和富文本 runs 两条渲染路径继续由页面组合。
