...
本报告以合并后的 `master@a216217` 为基线，并记录本分支的编辑器测试所有权与动作装配治理验证，使用仓库内的源文件计数、物理行数、依赖检索、`part` 库清单、架构测试、边界脚本、CI 门禁配置和近期合并历史进行审计。统计的“行数”是物理行数，用于识别维护热点，并不等价于业务复杂度或缺陷数量。
...
| P1 | 测试文件分布偏扁平 | 编辑器 presentation 测试已迁入 `test/features/drawing/presentation/`；跨域综合测试仍位于 `test/` 根目录 | 同域测试发现、所有权和批量执行定位成本提高 | 继续仅在改动关联 feature 时迁移，避免无收益的大搬家；跨 drawing/notes 测试保留根目录 |
...
| D（进行中） | 渐进整理测试所有权 | 已将 13 个编辑器 presentation 测试归属 `test/features/drawing/presentation/`，提升定位和审阅效率 | 仅迁移正在修改 feature 的相关测试，并保留 Flutter 递归发现；跨域综合测试继续留在根目录 | 不做只改路径的大搬家，不降低全量回归 |
| E | 模板策略的用户自定义持久化 | 让领域模板从内置枚举演进为受版本控制的配置 | 先定义模板 schema、升级策略和加密持久化语义 | 不在未定义迁移策略前直接写入用户数据 |

批次 A 已由 PR #28 和严格边界审计完成；批次 B 已通过 `DocumentCodec` 的 core 所有权迁移与旧路径兼容导出完成；批次 C 已完成形状缩放、图片裁剪和文字展示样式三个 presentation 叶子职责的收口；本批进一步完成与这些生产职责对应的 13 个测试所有权迁移，并以既有动作工厂全字段回归锁定装配边界。后续应继续在独立 PR 中治理跨域测试归属或组合根动作装配，且不得把 `BuildContext`、控制器、持久化或第二状态源下沉到纯协作者。
