...
| 优先级 | 风险 | 当前证据 | 影响 | 建议方向 |
|---|---|---|---|---|
| P0 | 编辑器组合根仍大 | `editor_page.dart` 为 1,124 行，多个 `part` 仍共享同一私有状态；形状缩放、图片裁剪和文字样式已由显式 presentation 协作者脱离重复分支 | 修改输入、overlay、工具栏时仍需理解大量隐式状态 | 继续按明确展示职责提取窄协作者/动作契约；不要仅为降低行数继续增加没有边界的 `part` |
...
| C（进行中） | `EditorPage` 从 `part` 私有库走向窄协作者契约 | 已将形状缩放、图片裁剪和文字样式收口为不可变输入/命名语义/纯 presentation 输出；后续可评估测试所有权和组合根动作装配 | 继续选择一组行为稳定的叶职责并先锁定交互不变量 | 不将 `BuildContext`、控制器和持久化下沉到纯状态对象 |
