...
| P0 | 命令与控制器存在冻结循环 | 架构测试明确冻结 `document_commands ↔ drawing_controller` 历史循环 | 命令基础设施难以独立复用，控制器仍含命令适配责任 | 单独设计“命令执行宿主”窄接口或把恢复回调迁移到会话宿主；迁移前先建立撤销/重做行为矩阵 |
...
|---|---|---|---|---|
| A | 命令—控制器冻结循环专项 | 消除最明确的依赖环，降低撤销体系耦合 | 先编制所有 `DocCommand` 的 redo/undo、恢复回调、缓存/通知时序矩阵 | 不重写历史功能，不改变用户撤销栈格式 |
...
建议优先启动**批次 A**。它是当前架构测试直接记录的唯一已知依赖环，且比继续拆分展示层更能改善长期演进能力；但它必须独立成 PR，并在实现前以行为矩阵锁定撤销、重做、缓存、选择修正和通知次序。
...
当前代码**是正常的、有架构和有工程门禁的 Flutter 项目**，并且经过连续重构后已经从早期的“编辑器和笔记模块集中式胶水代码”演进到以会话、纯领域对象、无状态服务和展示协作者为主的结构。其主要问题是规模增长后遗留的组合根、历史循环与容量约束，而不是架构缺失。

因此，最稳妥的策略不是推倒重来，而是继续维持现有的受保护分支流程，围绕依赖环、存储例外、大型组合根和测试所有权做小批次、可验证、可回滚的连续改造。报告中的路线图可以作为未来 3–5 个独立 PR 的优先级依据。
