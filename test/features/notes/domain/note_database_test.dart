...
void main() {
  group('NoteFieldDef', () {
    ...
    test('copyWith 只更新给定字段', () {
      const f = NoteFieldDef(
          id: 'status', name: '状态', type: NoteFieldType.select, options: ['A', 'B']);
    ...
    test('序列化往返', () {
      const f = NoteFieldDef(
          id: 's', name: '阶段', type: NoteFieldType.select, options: ['待办', '进行中']);
    ...
    test('相等性（含 options 顺序）', () {
      const a = NoteFieldDef(
          id: 's', name: '阶段', type: NoteFieldType.select, options: ['A', 'B']);
      const b = NoteFieldDef(
          id: 's', name: '阶段', type: NoteFieldType.select, options: ['A', 'B']);
      const c = NoteFieldDef(
          id: 's', name: '阶段', type: NoteFieldType.select, options: ['B', 'A']);
      expect(a, b);
      expect(a == c, isFalse);
    });
  });
  ...
  group('字段 CRUD', () {
    ...
    test('updateFieldOptions 更新 select 选项', () {
      final db = NoteDatabase.empty()
          .addField(const NoteFieldDef(
              id: 's', name: '状态', type: NoteFieldType.select, options: ['A', 'B']))
    ...
    test('removeField 删除字段并清掉所有记录里的对应 cell', () {
      var db = NoteDatabase.empty()
          .addField(const NoteFieldDef(id: 'n', name: '名称', type: NoteFieldType.text))
          .addField(const NoteFieldDef(id: 'a', name: '年龄', type: NoteFieldType.number))
  ...
  group('视图与排序', () {
    NoteDatabase db() => NoteDatabase.empty()
        ...
        .addField(const NoteFieldDef(id: 'n', name: '数量', type: NoteFieldType.number))
    ...
    test('数字字段用数值比较，非数字回退字符串', () {
      ...
      expect(d.setSort('n').sortedRecords.map((r) => r.id).toList(), ['y', 'x'],
  ...
  group('displayValue 显示字符串', () {
    NoteDatabase db() => NoteDatabase.empty()
        .addField(const NoteFieldDef(id: 't', name: '文本', type: NoteFieldType.text))
        .addField(const NoteFieldDef(id: 'n', name: '数字', type: NoteFieldType.number))
        .addField(const NoteFieldDef(id: 's', name: '选项', type: NoteFieldType.select))
        .addField(const NoteFieldDef(id: 'c', name: '勾选', type: NoteFieldType.checkbox))
        .addField(const NoteFieldDef(id: 'd', name: '日期', type: NoteFieldType.date))
        .addRecord(const NoteRecord(id: 'r', cells: {
          't': '你好',
          'n': 42,
          's': 'P1',
          'c': true,
    ...
    test('各字段类型的字符串形式', () {
      ...
      expect(dbx.displayValue(dbx.recordById('r')!, dbx.fieldById('d')!), '2026-01-01');
    });
  ...
  group('序列化与相等', () {
    test('toJson/fromJson 往返保持全量', () {
      ...
      db = db
          .addField(const NoteFieldDef(id: 'n', name: '名称', type: NoteFieldType.text))
          .addField(const NoteFieldDef(
              id: 's', name: '状态', type: NoteFieldType.select, options: ['待办', '完成']))
