  ...
  group('KekSessionCache', () {
    test('pointycastle(isolate) 与 cryptography Pbkdf2 逐字节一致', () async {
      ...
      for (final (pw, it, idx) in cases) {
        ...
        final actual = await cache.deriveKek(pw, salt, it);
      ...
      expect(
        await cache.deriveKek('big-iter-pw', salt100k, 100000),
    ...
    test('缓存命中：同输入两次派生结果一致且只存一条', () async {
      ...
      final a = await cache.deriveKek('same-pw', salt, 1000);
      final b = await cache.deriveKek('same-pw', salt, 1000);
      expect(a, orderedEquals(b));
      expect(cache.entryCount, 1);
      // 不同盐 → 不同条目、不同输出。
      final otherSalt = Uint8List.fromList(List.generate(16, (i) => i * 3 + 1));
      final c = await cache.deriveKek('same-pw', otherSalt, 1000);
    ...
    test('clear：条目清零；再派生结果不变（功能不受影响）', () async {
      ...
      final before = await cache.deriveKek('clear-pw', salt, 1000);
      expect(cache.entryCount, 1);
      cache.clear();
      expect(cache.entryCount, 0);
      final after = await cache.deriveKek('clear-pw', salt, 1000);
    ...
    test('并发同键去重：并发派生共享同一计算', () async {
      ...
      final results = await Future.wait([
        cache.deriveKek('concurrent-pw', salt, 50000),
        cache.deriveKek('concurrent-pw', salt, 50000),
        cache.deriveKek('concurrent-pw', salt, 50000),
      ]);
    ...
    test('LRU 上限封顶：条目数不超过 64', () async {
      for (var i = 0; i < 70; i++) {
        ...
        await cache.deriveKek('lru-pw-$i', salt, 100);
      }
      ...
      await cache.deriveKek('lru-pw-69', salt69, 100);
      expect(cache.entryCount, lessThanOrEqualTo(64));
    });
  });
}
