...
void main() {
  test('V-004：editor_core/notebook_domain 纯 Dart（无 Flutter/dart:io/dart:ui/平台包）', () {
    final files = [
    ...
    for (final f in files) {
      for (final line in _importsOf(f)) {
        for (final bad in ['package:flutter', 'dart:io', 'dart:ui', 'file_selector', 'path_provider', 'shared_preferences']) {
  ...
  test('V-005：V2 不得 import Legacy（features/drawing|notes|drawing_controller|editor_page|legacy/）', () {
    final files = [
      ..._dartFiles('packages/editor_core/lib'),
      ..._dartFiles('packages/notebook_domain/lib'),
      ..._dartFiles('lib/features/editor_v2'),
    ];
    final legacy = RegExp('features/drawing|features/notes|drawing_controller|editor_page|legacy/');
    for (final f in files) {
      for (final line in _importsOf(f)) {
  ...
  test('V-006：UI（app）不得直接访问文件/密钥/具体存储', () {
    final files = [..._dartFiles('lib/app'), ..._dartFiles('lib/features/editor_v2')];
    final forbidden = RegExp('dart:io|VaultService|MediaCryptoService|NotebookStorage');
    for (final f in files) {
      for (final line in _importsOf(f)) {
